    ...
    for cur_iter, (inputs, labels, index, time, meta) in enumerate(train_loader):
        ...
        if cfg.DETECTION.ENABLE:
            ...
            if cfg.DATA.MULTI_LABEL:
                ...
                # Compute the errors.
                k = 5 if cfg.MODEL.NUM_CLASSES >= 5 else cfg.MODEL.NUM_CLASSES 
...
def train(cfg):
    ...
    for cur_epoch in range(start_epoch, cfg.SOLVER.MAX_EPOCH):
        ...
        if train_loss < 0.3:
            logger.info(f"EARLY STOPPING: Loss {train_loss:.4f} < 0.3 at epoch {cur_epoch}")
            # Ensure we save the final model even if irregular epoch
            cu.save_checkpoint(
                cfg.OUTPUT_DIR,
                model,
                optimizer,
