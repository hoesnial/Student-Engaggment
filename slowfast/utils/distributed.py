...
def _init_distributed_training(num_gpus, shard_id):
    # Minimal implementation for single node / generic setup
    pass

def cat_all_gather(tensors):
    """
    Concatenates the results of all_gather for the provided tensors.
    """
    gather_list = [torch.zeros_like(tensors) for _ in range(torch.distributed.get_world_size())]
    torch.distributed.all_gather(gather_list, tensors)
