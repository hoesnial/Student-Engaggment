...
def image_decode(
    ...
    """
    Decode frames from a directory of images.
    container is the path to the directory.
    """
    try:
        # container is just the path
        path_to_video = container
        
        # List all frames
        all_frames = sorted([f for f in os.listdir(path_to_video) if f.endswith(('.jpg', '.png', '.jpeg'))])
        frames_length = len(all_frames)
        
        # Estimate duration (assuming 30fps if not known, but usually we just need frames_length for sampling)
        # We can try to guess FPS if needed, but for frame sequence it's often fixed or we assume target_fps
        fps = 30.0 # Default assumption for frame sequence if unknown
        ...
        for k in range(len(num_frames)):
            start_idx = int(start_end_delta_time[k, 0])
            end_idx = int(start_end_delta_time[k, 1])
            
            # Sampling indices based on sampling_rate
            # The get_multiple_start_end_idx gives range [start, end].
            # We need to sample `num_frames[k]` from this range with `sampling_rate[k]` stride (roughly)
            # Actually temporal_sampling function later does the sub-sampling usually?
            # BUT pyav_decode logic fetches specific frames.
            
            # Replicating logic similar to pyav_decode's fetching
            # Here we just load the specific frames needed
            
            seq = []
            # We need 'num_frames[k]' frames. 
            # In pyav_decode, it fetches a range and then temporal_sampling (called later in decode) handles stride?
            # Wait, pyav_decode returns `frames_out` which are TENSORS.
            # And `decode` calls `temporal_sampling` later on `frames`.
            
            # pyav_decode_stream fetches ALL frames in range [start_pts, end_pts].
            # Then `decode` function takes `frames_decoded` and does `temporal_sampling(frames, ...)`
            
            # So here we should strictly load frames in [start_idx, end_idx] 
            # OR we can be smarter and only load the ones `temporal_sampling` WILL need to save IO?
            # `temporal_sampling` does: index = torch.linspace(start_idx, end_idx, num_samples)
            
            # Let's pre-calculate indices to load to be efficient
            indices = np.linspace(start_idx, end_idx, num_frames[k]).astype(int)
            # Clamp
            indices = np.clip(indices, 0, frames_length - 1)
            
            loaded_frames = []
            for idx in indices:
                frame_path = os.path.join(path_to_video, all_frames[idx])
                # Load with cv2 (BGR)
            ...
            # Stack to (T, H, W, C)
            frames_torch = torch.as_tensor(np.stack(loaded_frames))
            frames_out[k] = frames_torch
...
def decode(
    ...
    try:
        if backend == "pyav":
            ...
            frames_decoded, fps, decode_all_video, start_end_delta_time = image_decode(
                container,
                sampling_rate,
                num_frames,
                clip_idx,
